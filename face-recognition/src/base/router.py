...
class APIRouter(_APIRouter):
    ...
    def api_route(
        ...
        if response_model and self.std_response_handling:  # Remodel the ResponseSchema
            ...
            response_model = self.parse(response_model)
            response_model = ResponseSchema(
                f"{self.prefix+path} Response Schema", ret_data=response_model, cust_type=type_
            )
        return super().api_route(path, response_model=response_model, **kwargs)

    def parse(self, obj):
        if python_ver[0] == 3 and python_ver[1] < 7:
            if type(obj) is typing.GenericMeta:
                if obj.__origin__ is dict:
                    return self.parse(obj.__args__[1])
                elif obj.__origin__ is list:
                    return self.parse(obj.__args__[0])
            else:
                return obj
        else:
            if type(obj) is typing._GenericAlias:
                if obj.__origin__ is dict:
                    return self.parse(obj.__args__[1])
                elif obj.__origin__ is list:
                    return self.parse(obj.__args__[0])
